package com.laotabu.web.controller.system;
...
//        user.setPassword(SecurityUtils.encryptPassword(user.getPassword()));
    ...
    public AjaxResult resetPwd(@RequestBody SysUser user) throws Exception {
        ...
        userService.checkUserDataScope(user.getUserId());
        // 前端加密的数据，先解密【RSA】
        user.setPassword(RSAUtil.decryptByPrivateKey(user.getPassword()));
//        user.setPassword(SecurityUtils.encryptPassword(user.getPassword()));
        // 再加密存储到数据库【bcrypt】
        user.setPassword(passwordEncoder.encode(user.getPassword()));
        user.setUpdateBy(getUsername());
     ...
     */
    @PreAuthorize("@ss.hasPermi('system:user:list')")
    @GetMapping("/deptCascader")
    public AjaxResult deptCascader(SysDept dept)
    {
        return success(deptService.deptCascader(dept));
    }
